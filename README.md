...
- 퍼즐 타일을 평면 아이콘이 아닌 떠 있는 마법 오브젝트 SVG 에셋으로 교체
- 자체 제작 프리미엄 캐주얼 오브젝트 SVG 24종 추가
- Texture Atlas 메타 파일 `dream-objects.atlas.json` 추가
- 2.5D 도서관 배경 `dream-library-25d.svg` 추가
...
- 클리어 보상 모달 추가
- 다음 스테이지 바로 시작 버튼 추가
- 월드맵용 자체 SVG 배경 `world-map` 추가
- 보상 연출용 자체 SVG `reward-badge` 추가
- 로비 진행판에 별 조각 카운터 추가
...
- 아기자기한 캐주얼 원화풍 자체 SVG 배경 `storybook-login` 추가
- 게임 시작 화면용 자체 SVG 배경 `lobby-garden` 추가
- 마스코트 사서 캐릭터 `librarian-momo` 자체 SVG 에셋 추가
...
7. Summary에 `Apply 꿈의 서고 v1.0.5 premium engine skeleton patch` 입력합니다.
...
v1.0.2 이후 들어간 자체 제작 기본 에셋입니다.

```text
public/assets/tiles/*.svg             24 files
public/assets/backgrounds/*.svg       4 files
public/assets/characters/*.svg        1 file
...
외부 저작권 에셋을 가져오지 않고, 프로젝트 안에서 바로 쓸 수 있는 자체 SVG 자원으로 구성했습니다. 로그인 배경, 로비 배경, 마스코트 캐릭터도 자체 SVG입니다.
