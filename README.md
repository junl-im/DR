...
7. Summary에 `Apply 꿈의 서고 v1.0.12 special rules boss telegraph and action stability patch` 입력합니다.
...
### v1.0.13 예정 - Collection Completion + Daily Board Upgrade Patch

- 컬렉션 도감 필터/정렬 추가
- 복원 프로젝트 완료 상태와 완료 보상 저장
- daily 리더보드 오늘/전체 탭 분리
...
1. 컬렉션/복원 완료 상태 저장
2. daily 리더보드 탭 분리
3. Texture Atlas lookup 확장
4. 보스 피격/콤보 컷인 추가 다듬기
