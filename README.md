...
- 이전 patch에서 남은 `package-lock.json`은 삭제 대상으로 기록: `DELETE_REMOVED_FILES_v1.0.11.txt`
...
7. Summary에 `Apply 꿈의 서고 v1.0.11 asset import back flow and actions stabilization patch` 입력합니다.
...
- `quality-check.yml`: push/PR 시 typecheck, SVG 금지, 카카오 handoff, 콘텐츠 데이터, 이미지 용량, 빌드 검사
...
### v1.0.12 예정 - Stage Rule Balance + Boss Attack Telegraph Patch

- 특수 타일 규칙을 실제 플레이 영향으로 확장: 안개 해제, 잠금 해제, 시간 봉인 보너스/패널티 정리
- 신규 import VFX를 실제 매치/폭발 파티클에 더 많이 배정
- 보스별 공격 예고 컷인과 카메라 흔들림 강도 추가 분리
- 뒤로가기/종료 팝업 UX를 PWA 설치 상태까지 추가 점검
- Actions 설치 방식을 lockfile 재생성 전략으로 더 안정화
- Texture Atlas 생성 스크립트 1차 추가
- 작은 화면 컬렉션/복원 패널 접기 추가
- 보스 공격 예고를 단순 흔들림에서 보스별 텔레그래프 UI로 분리
- 보스 피격 이펙트와 콤보 컷인 타이밍 추가 다듬기
- 컬렉션 도감 필터/정렬 추가
- 복원 프로젝트 완료 보상 상태 저장 추가
- daily 리더보드 표시를 오늘/전체 탭으로 분리
- 작은 화면에서 컬렉션/복원 패널 접기 UX 추가
- Texture Atlas JSON 기반 타일 lookup 실험 확장
