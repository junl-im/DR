...
if (!main.includes('v1054-store-collection-link-vfx') && !main.includes('v1050-summer-finale-event-vfx') && !main.includes('v1054-season-store-reward-burst')) errors.push('Missing summer patch token');
...
if (!main.includes('v1054-engine-design-gesture-qa') && !main.includes('v1050-summer-design-gesture-qa') && !main.includes('v1051-mobile-design-gesture-final')) errors.push('Missing season lobby drag token');
requireText(main, 'dy > 1.2', 'lobby drag threshold refinement');
requireText(main, 'dx * 0.28', 'lobby horizontal tolerance refinement');
if (!css.includes('v1054-store-collection-link-vfx') && !css.includes('v1050-summer-finale-event-vfx') && !css.includes('v1054-season-store-reward-burst')) errors.push('Missing summer CSS token');
if (!css.includes('next-goal-v1048-summer-pass') && !main.includes('next-goal-v1050-summer-finale') && !main.includes('v1054-season-store-reward-burst')) errors.push('Missing summer stage map CSS token');
if (!sw.includes('dream-library-cache-v1.0.86') && !sw.includes('dream-library-cache-v1.0.86') && !sw.includes('dream-library-cache-v1.0.86') && !sw.includes('dream-library-cache-v1.0.86')) errors.push('Missing service worker v1.0.48/v1.0.49 cache');
