...
if (!['1.0.47', '1.0.48', '1.0.49', '1.0.50', '1.0.50'].includes(pkg.version)) errors.push(`package version must be 1.0.47, 1.0.48, 1.0.49 or 1.0.50, got ${pkg.version}`);
...
if (!main.includes('v1049-summer-event-vfx') && !main.includes('v1050-summer-finale-event-vfx')) errors.push('Missing summer patch token');
...
if (!main.includes('v1049-season-vfx-gesture-qa') && !main.includes('v1050-summer-design-gesture-qa')) errors.push('Missing season lobby drag token');
requireText(main, 'dy > 1.2', 'lobby drag threshold refinement');
requireText(main, 'dx * 0.28', 'lobby horizontal tolerance refinement');
if (!css.includes('v1049-summer-event-vfx') && !css.includes('v1050-summer-finale-event-vfx')) errors.push('Missing summer CSS token');
if (!css.includes('next-goal-v1048-summer-pass') && !main.includes('next-goal-v1050-summer-finale')) errors.push('Missing summer stage map CSS token');
...
if (!sw.includes('texture-atlas-manifest-v1.0.48.json') && !sw.includes('texture-atlas-manifest-v1.0.49.json') && !sw.includes('texture-atlas-manifest-v1.0.50.json')) errors.push('Missing v1.0.48/v1.0.49 atlas manifest');
