...
for (const token of [
  ...
  if (!runtime.includes(token)) errors.push(`Missing v1.0.45 visual priority token: ${token}`);
}
...
if (!pages.includes('npm run check:board-focus-tempo-cache') || !quality.includes('npm run check:board-focus-tempo-cache')) errors.push('workflows must run v1.0.45 board focus tempo cache check.');
...
console.log('Board focus tempo cache check passed: board focus, boss tempo and slim cache v1.0.45 are active without removed UI.');
