...
if (!existsSync('public/assets/meta/texture-atlas-manifest-v1.0.45.json')) errors.push('v1.0.45 texture atlas manifest file missing.');
if (!workflows.includes('npm run check:mobile-board-reward-flow')) errors.push('workflows must run v1.0.45 mobile board reward flow check.');

if (errors.length) {
  console.error(`Mobile board reward flow check failed: ${errors.join('; ')}`);
  process.exit(1);
}
console.log('Mobile board reward flow check passed: v1.0.45 mobile board feel, boss polish and reward bridge are active without removed UI.');
