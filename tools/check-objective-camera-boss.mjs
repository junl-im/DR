...
if (!main.includes('dream-library-camera-guide-seen') || !css.includes('.board-camera-guide.camera-tutorial')) {
  errors.push('Large-board camera tutorial must be persisted and styled.');
}
...
const runtime = `${html}\n${main}\n${renderer}\n${css}`;
for (const token of ['board-minimap', 'boardMinimap', '보드 레이더', '레이더 탭']) {
  if (runtime.includes(token)) errors.push(`Minimap must stay removed: ${token}`);
}
...
console.log('Objective camera boss check passed: markers, first-use camera guide and boss warning variants are active without minimap.');
