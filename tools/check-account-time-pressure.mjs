...
for (const token of [
  ...
  if (!runtime.includes(token)) errors.push(`Missing v1.0.43 account/time pressure token: ${token}`);
}
...
if (!pages.includes('npm run check:account-time-pressure')) errors.push('github-pages workflow must run v1.0.43 account/time pressure check.');
if (!quality.includes('npm run check:account-time-pressure')) errors.push('quality workflow must run v1.0.43 account/time pressure check.');
